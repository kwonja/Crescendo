...
export default function Profile({userInfo,setProfileImage,setNickname,setIntroduction,userId}:ProfileProps) {
  ...
  const handleSaveClick = () => {
    setIsEditing(false);
    Promise.all([modifyIntroductionAPI(userId), modifyNicknameAPI(userId), modifyProfileAPI(userId)]);
  };
    ...
    <>
      <div className="profile">
        <div className="img">
          ...
          {isEditing ? ( <><label htmlFor="profile" className='editimg'>업로드</label>
            <input
              id="profile"
              className='hidden'
              type="file"
     ...
     {
       userId === getUserId() ? <>{isEditing ? (
        <div className='profile_save'>
          <button className="w-1/4 bg-mainColor"onClick={handleSaveClick}>저장</button>
         <button className="w-1/4 bg-subColor" onClick={handleCancelClick}>취소</button>
        </div>
      ) : (
      ...
      {
        newisFollowing ? <div className='profile_edit'>
        <button onClick={()=> setNewisFolowing(false)}>Unfollow</button>
        </div> : <div className='profile_edit'>
